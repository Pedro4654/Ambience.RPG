...
@section('title', 'Seguidores de ' . $usuario->username . ' - Ambience RPG')
  ...
  <div class="followers-content">
    ...
    <div class="followers-header">
      <div class="header-content">
        <div class="header-info">
          ...
          <p>{{ $usuario->username }} tem {{ $seguidores->total() }} seguidor(es)</p>
        </div>
          ...
          <div class="user-card">
            <div class="user-info">
              <a href="{{ route('perfil.show', $usuario_seguidor->username) }}">
                @if($usuario_seguidor->avatar_url)
                  <img src="{{ $usuario_seguidor->avatar_url }}" alt="{{ $usuario_seguidor->username }}" class="user-avatar">
                @else
                  <div class="avatar-default">{{ strtoupper(substr($usuario_seguidor->username, 0, 1)) }}</div>
                @endif
              </a>
              
              <div class="user-details">
                <a href="{{ route('perfil.show', $usuario_seguidor->username) }}" class="user-name">
                  {{ $usuario_seguidor->username }}
                </a>
                <p class="user-bio">{{ $usuario_seguidor->bio ?? 'Sem bio' }}</p>
                ...
                <div class="user-stats">
                  <span class="user-stat">
                    ...
                    {{ $usuario_seguidor->posts()->count() ?? 0 }} posts
                  </span>
                  <span class="user-stat">
                    <svg class="svg-icon" viewBox="0 0 1024 1024" style="width: 13px; height: 13px;">
                      <path d="M768 576c70.4 0 128-57.6 128-128s-57.6-128-128-128-128 57.6-128 128 57.6 128 128 128z m128 64h-256c-70.4 0-128 57.6-128 128v64h512v-64c0-70.4-57.6-128-128-128zM384 576c70.4 0 128-57.6 128-128s-57.6-128-128-128-128 57.6-128 128 57.6 128 128 128z m128 64H256c-70.4 0-128 57.6-128 128v64h384v-64c0-70.4-57.6-128-128-128z"/>
                    </svg>
                    {{ $usuario_seguidor->total_seguidores ?? 0 }} seguidores
                  </span>
                </div>
              </div>
            </div>
                ...
                <div class="action-buttons">
                  @if($ja_sigo)
                    <form action="{{ route('perfil.deixar_de_seguir', $usuario_seguidor->id) }}" method="POST">
                      @csrf
                      @method('DELETE')
                      <button type="submit" class="btn btn-unfollow" onclick="return confirm('Deixar de seguir {{ $usuario_seguidor->username }}?')">
                        <svg class="svg-icon" viewBox="0 0 1024 1024">
    ...
    @else
      <div class="empty-state">
        ...
        <h2 class="empty-state-title">Sem seguidores ainda</h2>
        <p class="empty-state-description">{{ $usuario->username }} ainda não tem seguidores</p>
